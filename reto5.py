...
def iniciar_sesion(registros_existentes):
    ...
    if usuario_encontrado:
        if usuario_encontrado.get("contraseña") == contrasena_ingresada:
            # Añade 'sesion_activa' solo al diccionario temporal 'usuario_encontrado'
            ...
            # Retorna el usuario encontrado con la sesión activa
    ...
    # Retorna None si el inicio de sesión falla
    return None
...
def agendar_consulta(registros_existentes, usuario_logueado):
    ...
    if not usuario_logueado or usuario_logueado.get('rol que desempeña') != 'paciente':
    ...
    for usuario in registros_existentes:
        if usuario.get("ID") == id_medico and usuario.get("rol que desempeña") == "médico":
